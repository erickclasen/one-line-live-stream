...
DEST_DIR="/var/www/html/live"
REMOTE_SERVER="user@laptop"
...
do
    ...
    if [[ "$FILE" =~ $TS_PATTERN ]]; then
        ...
        RSYNC_CMD="rsync -avz --progress \"$SOURCE_DIR/$FILE\" \"$REMOTE_SERVER:$DEST_DIR/\""
        ...
        RSYNC_CMD="rsync -avz --progress \"$SOURCE_DIR/$M3U8_FILE\" \"$REMOTE_SERVER:$DEST_DIR/\""
        echo "Running command: $RSYNC_CMD"
